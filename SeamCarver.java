...
public class SeamCarver {
   ...
   public Integer[] findHorizontalSeam(){
	// sequence of indices for horizontal seam
	   ...
	   this.img = transposePic(this.img);
	   ...
	   Integer[] horizSeam = TransIndex2OrigIndex(oldImg,horizSeamT);
   ...
   private static Integer[] TransIndex2OrigIndex(Picture nonTran, Integer[] linIndexT){
	  //Maps linear indices in a transposed image to linear indices in non-transposed image
	   Integer[] regIndices = new Integer[linIndexT.length];
	   int[] k = new int[2];
	   
	   //the transposed height/width are width/height of original
	   int heightT = nonTran.width();
	   int widthT  = nonTran.height();
	   
	   for(int i = 0; i < linIndexT.length; i++){
		   //convert back to regular indices in transposed image
		   k = ind2sub(heightT, widthT, linIndexT[i]);
		   // go from regular indices in transposed image, to linear indices in non-transposed image
		   int newLinearIndeces = sub2ind(k[0],k[1],nonTran.width());
		   regIndices[i] = newLinearIndeces;
	   }
	   return regIndices;
   }
   private static Integer[] tran2origIndex(int width, int height, Integer[] linIndexT){
	  //Maps linear indices in a transposed image to linear indices in non-transposed ("original") image
	   //takes in width and height of non-transposed image and the linear indices in the transposed image
	   
	   Integer[] origInd = new Integer[linIndexT.length];
	   int[] k = new int[2];
	   ...
	   for(int i = 0; i < linIndexT.length; i++){
		   ...
		   k = ind2sub(heightT, widthT, linIndexT[i]);
		   // go from 2D subscripts in transposed image, to linear indices in non-transposed image
		   int newLinearIndeces = sub2ind(k[0],k[1], width);
   ...
   private void relax(int row, int col, double[][] distTo, int[][] edgeTo){
...
//   public    void removeHorizontalSeam(int[] seam)   // remove horizontal seam from current picture
   ...
   private static Picture transposePic(Picture old){
	   Picture transpose = new Picture(old.height(),old.width());
	   
	   for(int i = 0; i < old.height(); i++){
		   for(int j = 0; j < old.width(); j++){
			   transpose.set(i, j, old.get(j, i));
		   }
	   }
	   return transpose;
   }
   ...
   private Integer[] verticalSeam(int bottomRow, int bottomCol, int[][] edgeTo){
	   ...
	   while(node>=0){
		   ...
		   int[] indeces = ind2sub(height(),width(),node);
		   //index edgeTo with found indeces for next node in chain
		   node = edgeTo[indeces[0]][indeces[1]];
	   }
   ...
   private static int sub2ind(int row, int column, int width){
		return (width*row + column);	
	}
   private static int[] ind2sub(int height, int width, int LinearIndex){
   ...
   public static void main(String args[]){
	 ...
	 //test findVerticalSeam
	 		...
	 		Integer[] correctSea = {sub2ind(4,2,6), sub2ind(3,2,6), sub2ind(2,3,6), sub2ind(1,4,6),sub2ind(0,3,6)};
	...
	// test deltaX deltaY	   
	 ...
	 // Test TransIndex2RegIndex
	 	...
	 	System.out.println("transIndices from method:" + Arrays.toString(TransIndex2OrigIndex(new Picture(3,3),transIndices)));
	 	System.out.println(Arrays.deepEquals(TransIndex2OrigIndex(new Picture(3,3),transIndices), transIndices) );
